...
const
  InternetProfessionalVersion = 1.16;

resourcestring
  sLongVersion = 'Version %.2f Beta';
  sShortVersion = 'v%.2fß';
...
{ RFC-2396 "Uniform Resource Identifiers (URI): Generic Syntax"   }
  ...
  function BuildURL(const Old, New: string): string;
...
begin
  ...
  { Main processing }
  Result := DOSToNetPath(Old);

  Initialize(OldAddrRec);
  Initialize(NewAddrRec);

  IpParseURL(Old, OldAddrRec);
  IpParseURL(New, NewAddrRec);
...
{End !!.14}
    ...
    Result := Scheme + OldAddrRec.Authority + Path +                     {!!.12}
      Copy(New, RelPos, Length(New) - RelPos + 1);
