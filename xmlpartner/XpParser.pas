...
//    FCurrentPath : string;                                           {!!.52}
    ...
    function IsRealWhitespace(const cVal : DOMChar) : Boolean;             {!!.pk}
...
{$ENDIF}
    ...
    property IgnoreXMLVersion : Boolean read FIgnoreXMLVersion write FIgnoreXMLVersion;

    property KeepLF : Boolean read FKeepLF write FKeepLF;
  end;
...
var
  j           : Integer;
  NumToDelete : Integer;
  WhiteSpaceEquivalent : Boolean;                                               {!!.55 - Start}
begin
  if (FDataBufferPos > 0) then begin
    ...
    while (j <= FDataBufferPos) do begin
      if FKeepLF then
        WhiteSpaceEquivalent := IsRealWhiteSpace(FDataBuffer[j])
      else
        WhiteSpaceEquivalent := IsWhiteSpace(FDataBuffer[j]);

      if WhiteSpaceEquivalent then begin
        ...
        if FKeepLF then
          ...
          WhiteSpaceEquivalent := IsWhiteSpace(FDataBuffer[j]);
        while ((j <= FDataBufferPos) and WhiteSpaceEquivalent) do begin
...
{--------}
function TXpParser.IsRealWhitespace(const cVal : DOMChar) : Boolean;       {!!.52}
begin
{Rewritten !!.51}
  Result := (cVal = #$20) or (cVal = #$09);
end;
{--------}
